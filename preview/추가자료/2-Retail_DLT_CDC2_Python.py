...
@dlt.table(name="customer_bronze2",
...
@dlt.view(name="customer_bronze_clean_v2",
...
def customer_bronze_clean_v2():
  return dlt.read_stream("customer_bronze2") \
...
dlt.create_target_table(name="customer_silver2",
...
dlt.apply_changes(
  target = "customer_silver2", #The customer table being materilized
  source = "customer_bronze_clean_v2", #the incoming CDC
...
dlt.create_target_table(name="customer_silver3",
  comment="Clean, merged customers",
  table_properties={
    "quality": "silver"
  }
...
dlt.apply_changes(
  target = "customer_silver3", #The customer table being materilized
  source = "customer_silver2", #the incoming CDC
  keys = ["id"], #Primary key to match the rows to upsert/delete
  sequence_by = col("operation_date"), #deduplicate by operation date getting the most recent value
  #apply_as_deletes = expr("operation = 'DELETE'"), #DELETE condition
  #except_column_list = ["operation", "operation_date", "_rescued_data"], # drop metadata columns,
...
def customer_gold():
    return dlt.read_stream('customer_silver2').select('id', 'email', 'address')

# COMMAND ----------

@dlt.table(name="customer_gold2",
  table_properties={
    "quality": "gold"
  }
)
...
def customer_gold2():
    return dlt.read_stream('customer_gold').select('id', 'email', 'address')
