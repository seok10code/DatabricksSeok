...
spark.sql(f"DROP DATABASE IF EXISTS main.gamedb CASCADE")
spark.sql(f"CREATE DATABASE main.gamedb")
spark.sql(f"USE main.gamedb")
...
trainDF = spark.read.csv(f'/FileStore/tmpdir/train.csv',header=True,escape='"',multiLine=True)
trainDF.write \
  .format("delta") \
  .mode("overwrite") \
  .saveAsTable("toxicity_training") # uses user-specific databases set in notebook config 

testDF = spark.read.csv(f'/FileStore/tmpdir/test.csv',header=True,escape='"',multiLine=True)
testDF.write \
  .format("delta") \
  .mode("overwrite") \
