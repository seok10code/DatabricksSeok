...
peopleDF = spark.read.format("parquet").load("/mnt/training/dataframes/people-10m.parquet/")
...
peopleDF.createOrReplaceTempView("PeoplesTableFMI")
...
display(spark.sql("SELECT * FROM PeoplesTableFMI WHERE firstName = 'Chris'"))
...
# MAGIC %sql
# MAGIC SELECT * FROM PeoplesTableFMI WHERE firstName = 'Chris'
