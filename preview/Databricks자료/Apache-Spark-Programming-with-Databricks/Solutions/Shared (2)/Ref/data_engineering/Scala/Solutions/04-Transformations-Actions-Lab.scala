...
val INPUT_FILE = "dbfs:/mnt/training/initech/products/dirty.csv"

val initialDF = spark.read         // The DataFrameReader
  .option("header", true)          // Use first line of all files as header
  .option("inferSchema", true)     // Automatically infer data types
  .csv(INPUT_FILE)                 // Creates a DataFrame from CSV after reading in the file


val finalDF = initialDF
...
val OUTPUT_FILE = s"$userhome/training/initech/Products.parquet"

// COMMAND ----------

// ANSWER

finalDF.write.mode("overwrite").parquet(OUTPUT_FILE)
...
val parquet = OUTPUT_FILE

assert (exists(parquet), s"${parquet} does not exist")
