...
lazy val connectionString = ConnectionStringBuilder(pcString)
  .setEventHubName(uniqueEHName)
  .build

lazy val ehConf = EventHubsConf(connectionString)
...
val editsDF = spark.readStream     // Get the DataStreamReader
  ...
  .options(ehConf.toMap)           // Event Hubs options as a map
