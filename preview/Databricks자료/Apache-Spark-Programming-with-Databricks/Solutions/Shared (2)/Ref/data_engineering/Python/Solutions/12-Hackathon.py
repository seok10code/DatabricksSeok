...
# MAGIC # ANSWER
# MAGIC
# MAGIC This can be done using Azure Data Factory or Azure Databricks. Below is the solution using Azure Databricks.

# COMMAND ----------

# ANSWER

mayDF = spark.read.parquet("/mnt/partnerdata-external/internetSales/05/")

(mayDF.write.format("parquet")
  .partitionBy("OrderDate")
  .mode("append")
  .save("/mnt/partnerdata-internal/05/"))
...
# ANSWER
# Load the Data from the Internal Source
from pyspark.sql.functions import col, unix_timestamp
mayDF = spark.read.parquet("/mnt/partnerdata-internal/05/")
transformedDF = myDF.withColumn("OrderDateKey", unix_timestamp(col("OrderDate")))
display(transformedMayDF)

# COMMAND ----------

# ANSWER
# Select and transform the data to match the schema of the Data Warehouse

from pyspark.sql.functions import col, unix_timestamp

transformedMayDF = (mayDF
  .select(col("ProductId").alias("ProductKey"),
          unix_timestamp(col("OrderDate")).alias("OrderDateKey").cast("timestamp"),
          col("DueDate").alias("DueDateKey"),
          col("ShipDate").alias("ShipDateKey"),
          col("Customer.CustomerId").alias("CustomerKey"),
          col("Promotion").alias("PromotionKey"),
          col("Currency").alias("CurrencyKey"),
          col("SalesTerritory.SalesTerritoryId").alias("SalesTerritoryKey"),
          ...
          col("UnitPrice"),
          col("ExtendedAmount"),
          col("UnitPriceDiscountPct"),
          col("DiscountAmount"),
          col("ProductStandardCost"),
          col("TotalProductCost"),
          col("SalesAmount"),
          col("TaxAmt"),
          col("Freight"),
          col("CarrierTrackingNumber"),
          col("CustomerPONumber"))
)
...
# ANSWER
(transformedMayDF.write
  .mode("overwrite")
  .jdbc(JDBC_URL_DATA_WAREHOUSE, "FactInternetSalesStaging", mode="overwrite"))
