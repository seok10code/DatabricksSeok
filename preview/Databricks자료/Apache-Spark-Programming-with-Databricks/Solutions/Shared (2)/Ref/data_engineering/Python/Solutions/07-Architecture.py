...
# MAGIC
# MAGIC Databricks Delta is a unified data management system that brings reliability and performance (10-100x faster than Spark on Parquet) to cloud data lakes.  Delta's core abstraction is a Spark table with built-inreliability and performance optimizations.
# MAGIC
# MAGIC You can read and write data stored in Databricks Delta using the same familiar Apache Spark SQL batch and streaming APIs you use to work with Hive tables or DBFS directories. Databricks Delta provides thefollowing benefits:
# MAGIC
# MAGIC - **Massive Scale** // made possible by decoupling compute and storage
# MAGIC - **Reliability** // ACID Transactions and Data Validation
# MAGIC - **Performance** // Data Indexing and Caching (10-100x)
# MAGIC   - *Statistics and data skipping* - Reads are 10-100x faster when statistics are tracked about the data in each file, allowing Delta to avoid reading irrelevant information.
# MAGIC   - *Automatic file management* - Data access speeds up by organizing data into large files that can be read efficiently.
# MAGIC - **Low-Latency** // Real-Time streaming ingestion
