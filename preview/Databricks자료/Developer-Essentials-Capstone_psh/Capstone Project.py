...
def writeToBronze(sourceDataframe, bronzePath, streamName):
  (sourceDataframe
    .withColumn('eventDate',to_date((col('eventParams.client_event_time').alias('client_event_time')),'yyyy-MM-dd'))
...
def bronzeToSilver(bronzePath, silverPath, streamName, lookupDF):
  (spark.readStream
    ...
    .select(col('eventParams.device_id').alias('device_id'), 
            ...
            col('eventDate'))
    .join(lookupDF, on='device_id', how='inner')
    .writeStream 
    .format("delta")        
...
# TODO
spark.sql("""CREATE TABLE IF NOT EXISTS mobile_events_delta_gold USING delta LOCATION 'dbfs:/dbacademy/ecart77@cloocus.com/developer-essentials-capstone/output/gold'""".format(outputPathGold))
